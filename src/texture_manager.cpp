...
namespace ld40 {
	void TextureManager::load_sheet(std::string sheet) {
		...
		for (auto &texture : json.at(u8"textures")) {
			this->textures.emplace(texture.at(u8"species"), std::make_pair<const sf::Texture*, sf::Rect<float>>(sheet_texture.first->second.get(), sf::Rect<float>(texture.at(u8"x"), texture.at(u8"y"), 128.0f, 128.0f)));
		}
		return;
	}

	std::pair<const sf::Texture*, sf::Rect<float>> TextureManager::get_texture(std::string texture) const {
		if (this->textures.count(texture)) {
			return this->textures.at(texture);
		}
		return std::make_pair<const sf::Texture*, sf::Rect<float>>(nullptr, sf::Rect<float>());
	}
